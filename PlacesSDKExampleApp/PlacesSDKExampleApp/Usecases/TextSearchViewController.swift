...
//
//  Created by Zeeshan Khundmiri on 08/08/24.
//
...
class TextSearchViewController: BaseViewController {
    ...
    lazy var searchTextField: UITextField = {
        ...
        let placeholderText = "Search for a popular places"
    ...
    lazy var titleLabel: UILabel = {
        ...
        label.text = "Search Popular Places"
    ...
    lazy var exampleLabel: UILabel = {
        ...
        label.text = "Ex:  Cafes in Koramangala, park in kormangala etc..."
        label.numberOfLines = 0
        return label
    }()
    ...
    var textSearchResponse: TextSearchResponseModel?
    ...
    func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            exampleLabel.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -16),
            exampleLabel.topAnchor.constraint(equalTo: self.titleLabel.bottomAnchor, constant: 16),
            
            searchTextField.topAnchor.constraint(equalTo: self.exampleLabel.bottomAnchor, constant: 16),
    ...
    @objc func searchPlaceButtonAction() {
        ...
        self.placeService.textSearchAPI(input: text, location: location) { [weak self] result in
            DispatchQueue.main.async {
                self?.hideActivityIndicator()
                self?.view.endEditing(true)
                switch result {
                case .success(let success):
                    if success.predictions?.count ?? 0 > 0 {
                        self?.textSearchResponse = success
...
extension TextSearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.textSearchResponse?.predictions?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: .subtitle, reuseIdentifier: nil)
        guard let results = textSearchResponse?.predictions else { return UITableViewCell()}
